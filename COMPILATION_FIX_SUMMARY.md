...
5. **C++ 标准版本问题**: 使用 C++11 标准但代码中使用了指定初始化器语法
6. **C++20 兼容性问题**: C++20 引入了 `char8_t` 类型，导致 `u8` 字符串字面量类型不兼容
7. **未定义的函数引用**: llama.cpp 中引用了许多在其他源文件中定义的函数
...
- **升级 C++ 标准**: 从 C++11 升级到 C++17 以支持指定初始化器，避免 C++20 的 `char8_t` 兼容性问题
...
选择 C++17 而不是 C++20 的原因：

1. **避免 `char8_t` 问题**: C++20 引入了 `char8_t` 类型，使得 `u8` 字符串字面量从 `const char*` 变为 `const char8_t*`，导致类型不匹配错误
2. **保持兼容性**: C++17 仍然支持我们需要的现代 C++ 特性，包括指定初始化器（虽然会有警告）
3. **更广泛的编译器支持**: C++17 在更多环境中得到支持，兼容性更好
4. **避免修改 llama.cpp 源码**: 使用 C++17 可以避免修改第三方库的源代码
